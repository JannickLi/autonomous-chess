#!/usr/bin/env python3
"""ROS2 TCP Bridge Server for animated-knight backend.

Run this script with system Python (where ROS2 is sourced) to allow the
backend (running in a venv with a different Python version) to communicate
with ROS2 topics over TCP.

Usage:
    source /opt/ros/humble/setup.bash   # or jazzy, iron, etc.
    python3 ros_bridge_server.py [--host 0.0.0.0] [--port 9998]

The backend should then be configured with:
    OPERATION_MODE=ros
    ROS_TCP_HOST=localhost
    ROS_TCP_PORT=9998
...
logger = logging.getLogger("chess_agent_bridge")
...
# ---------------------------------------------------------------------------
# Maps topic names to a tag used for publisher/subscriber creation and
# message serialization.  "empty" = std_msgs/Empty, "string" = std_msgs/String,
# and the chess_msgs types when available.

_TOPIC_TYPES: dict[str, str] = {
    # Trigger topics (Empty)
    "/capture": "empty",
    "/chess/capture": "empty",
    "/chess/board/capture": "empty",  # legacy alias
    "/robot_home": "empty",
...
def _board_state_to_dict(msg: Any) -> dict[str, Any]:
    """Convert chess_msgs/BoardState → JSON-serialisable dict."""
    return {
        "success": msg.success,
        "fen": msg.fen,
...
def _move_command_to_dict(msg: Any) -> dict[str, Any]:
    """Convert chess_msgs/MoveCommand → dict."""
    return {
...
def _move_result_to_dict(msg: Any) -> dict[str, Any]:
    """Convert chess_msgs/MoveResult → dict."""
    return {
        "move_uci": msg.move_uci,
        "success": msg.success,
        "error": msg.error,
        "execution_time": float(msg.execution_time_sec),
    }


def _dict_to_move_command(data: dict[str, Any]) -> "MoveCommand":
    """Convert dict → chess_msgs/MoveCommand."""
    msg = MoveCommand()
...
# ---------------------------------------------------------------------------
# AgentOpinion / AgentOpinions conversion helpers
# ---------------------------------------------------------------------------

def _agent_opinion_to_dict(msg: Any) -> dict[str, Any]:
    """Convert chess_msgs/AgentOpinion → dict."""
    return {
        "piece_type": msg.piece_type,
        "piece_color": msg.piece_color,
...
def _agent_opinions_to_dict(msg: Any) -> dict[str, Any]:
    """Convert chess_msgs/AgentOpinions → dict."""
    return {
...
def _dict_to_agent_opinion(data: dict[str, Any]) -> "AgentOpinion":
    """Convert dict → chess_msgs/AgentOpinion."""
    print(f"_dict_to_agent_opinion: {data}")
    msg = AgentOpinion()
...
def _dict_to_agent_opinions(data: dict[str, Any]) -> "AgentOpinions":
    """Convert dict → chess_msgs/AgentOpinions."""
    msg = AgentOpinions()
    msg.opinions = [
...
def _agent_request_to_dict(msg: Any) -> dict[str, Any]:
    """Convert chess_msgs/AgentRequest → dict."""
    return {
        "fen": msg.fen,
...
def _dict_to_agent_request(data: dict[str, Any]) -> "AgentRequest":
    """Convert dict → chess_msgs/AgentRequest."""
    msg = AgentRequest()
    msg.fen = data.get("fen", "")
...
class ChessBridgeNode(Node):
    """ROS2 node that bridges JSON/TCP ↔ ROS2 topics."""

    def __init__(self) -> None:
        super().__init__("chess_agent_tcp_bridge")
        ...
        # Publishers (backend → ROS): created lazily on first publish request
        self._pub_map: dict[str, Any] = {}
        self._pub_lock = threading.Lock()

        # Subscribers (ROS → backend): keyed by topic, value = rclpy subscription
        self._subscribers: dict[str, Any] = {}
        self._sub_lock = threading.Lock()

        # Callbacks to call when a ROS message arrives: topic → list of callables
        self._ros_callbacks: dict[str, list] = {}
        self._cb_lock = threading.Lock()

        self._volatile_qos = volatile_qos
        self.get_logger().info("ChessBridgeNode initialized")
    ...
    def add_ros_subscription(
        self, topic: str, callback: Any
    ) -> None:
        """Subscribe to a ROS2 topic and forward messages to callback."""
        with self._cb_lock:
        ...
        with self._sub_lock:
            if topic in self._subscribers:
                return  # Already subscribed at ROS level
    ...
    def remove_ros_callback(self, topic: str, callback: Any) -> None:
        """Remove a specific callback for a topic."""
        with self._cb_lock:
...
class ClientHandler(threading.Thread):
    ...
    def send(self, message: dict[str, Any]) -> None:
        """Send a JSON line to this client (thread-safe)."""
    ...
    def _on_ros_message(self, topic: str, data: dict[str, Any]) -> None:
        """Forward a ROS message to this client."""
...
class TCPBridgeServer:
    ...
    def __init__(self, host: str, port: int, node: ChessBridgeNode) -> None:
        self._host = host
        self._port = port
        self._node = node
    ...
    def run(self) -> None:
        ...
        logger.info(f"ROS TCP bridge listening on {self._host}:{self._port}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ROS2 TCP bridge server for animated-knight backend"
    )
    parser.add_argument("--host", default="0.0.0.0", help="Bind host (default: 0.0.0.0)")
    parser.add_argument("--port", type=int, default=9998, help="Bind port (default: 9998)")
